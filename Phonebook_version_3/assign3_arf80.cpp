...
//This program adds, displays, searches, deletes, and sorts contacts in a phone book.
#include <iostream>
#include<fstream>
#include<string>
#include <algorithm>
...
int main() {
    ...
    Contact *ptr = new Contact[counter];
    Contact *c = new Contact;


    while (getline(fin, c->firstName, '~')) {
       // Contact *phoneBook = new Contact;
        getline(fin, c->lastName, '~');
        getline(fin, decoy1, '~');
        c->phoneNumber = stol(decoy1);
        getline(fin, decoy1);
        decoy2 = stoi(decoy1);
        c->phoneType = static_cast <PhoneType>(decoy2);
        ptr[friends] = *c;
        friends++;
    ...
    while (menuLoop) {
        ...
        switch (choice) {
            case 1:
                displayList(ptr, friends, results, caseSearch);
                break;
            case 2:
                enterContact(ptr, friends);
                break;
            case 3:
                caseSearch = true;
                ...
                results = binarySearch(ptr, friends, value);
                ...
                break;
            case 4:
                deleteContact(ptr, friends);
                break;
            case 5:
                exitFile(fin, ptr, friends);
